...
/// <summary>
/// Parser and Serializer for WebVTT (.vtt / S_TEXT/WEBVTT) format.
/// </summary>
public static class VttCodec
{
    ...
    public static SubtitleDocument Parse(string content)
    {
        ...
        while (i < lines.Length)
        {
            ...
            if (string.IsNullOrEmpty(line) || line.StartsWith("WEBVTT", StringComparison.OrdinalIgnoreCase) || line.StartsWith("NOTE", StringComparison.OrdinalIgnoreCase))
            {
                i++;
                continue;
            }
            ...
            if (match.Success)
            {
                var startTime = ParseVttTimestamp(match.Groups[1].Value, match.Groups[2].Value, match.Groups[3].Value, match.Groups[4].Value);
                var endTime = ParseVttTimestamp(match.Groups[5].Value, match.Groups[6].Value, match.Groups[7].Value, match.Groups[8].Value);
    ...
    public static string Serialize(SubtitleDocument document)
    {
        ...
        foreach (var cue in document.Cues.OrderBy(c => c.StartTime))
        {
            ...
            sb.Append(" --> ");
            sb.AppendLine(FormatVttTimestamp(cue.EndTime));
    ...
    public static string FormatVttTimestamp(TimeSpan time)
    {
    ...
    private static TimeSpan ParseVttTimestamp(string h, string m, string s, string ms)
    {
        int hours = string.IsNullOrEmpty(h) ? 0 : int.Parse(h, CultureInfo.InvariantCulture);
